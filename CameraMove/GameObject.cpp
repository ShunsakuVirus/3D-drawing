...
void line(int x1, int y1, int x2, int y2, Color c) {
	...
	void Circle::SetLocalPosition(float _x, float _y, float _z) {
		this->local_vtx = Vector4::Vector4(_x, _y, _z, 1);
	}
	...
	void Circle::Rotation(float _x, float _y, float _z) {
		this->rotX = Matrix4x4::rotateX(_x);
		this->rotY = Matrix4x4::rotateY(_y);
		this->rotZ = Matrix4x4::rotateZ(_z);
	}

	void Circle::Translate(float _x, float _y, float _z) {
		this->translate = Matrix4x4::translate(_x, _y, _z);
	}

	void Circle::Draw(Color c, Camera cam) {
		Vector4 work;
		work = work * this->rotX*this->rotY*this->rotZ* this->translate;
		this->x = work.x;
		this->y = work.y;
		...
		this->vtx = Vector4::screenTransformation(work, 960, 540);
		if (this->vtx.z < 1.0F && this->vtx.z != 0.0F) {
			drawCircle(this->vtx.x, this->vtx.y, this->size, this->size, 8, 2, c);//(this->vtx.x, this->vtx.y, this->size, c);
		}
	}
	...
	void Particle::display(Camera cam) {
		...
		this->point.Translate(this->location.x, this->location.y, this->location.z);
		this->point.Draw(Color(this->hu, 255, 255, this->lifespan), cam);
	}
	...
	Firework::Firework(Random &dice) {
		this->hu = dice(1,255);
		this->firework = Particle(dice(-10.0F,10.0F),0.0F,dice(-10.0F,10.0F), this->hu, dice);
